...
A Plex tool ends at an identifier: out of the box it plays nothing, so the
identifier *is* the product of every command -- a ``play`` behind an explicit
operator gate is the one exception. There are six strings in circulation that
all look like one:
...
command that takes it labels it ``media_id``. The ``guid`` is printed too, under
its own label, because it is *usually* the only identifier that survives a
re-match: a ``ratingKey`` is a row number in one server's database and moves when
...
**What changed on the way here, and what did not.** The forms, the regular
expressions, the notes and the refusals are the tool's. What moved is where the
...
from ..errors import UsageError, note, run
...
def validate_rating_key(raw: str, *, command: Sequence[str]) -> str:
    """Accept a rating key, and reject anything that only looks like one.

    ``command`` is the words of the caller's own invocation *after* the tool
    name -- ``("track",)``, ``("playlist", "add", "'Example'")`` -- because the
    recovery this raises is data and the name is supplied when it is rendered.
    A bare string is refused rather than accepted: :func:`~axi_toolkit.errors.run`
    would take one apart into its characters, and the resulting line would look
    plausible right up until somebody read it.
    """
    ...
    if _GUID.match(value):
        raise UsageError(
            ...
            recovery=(
                note(
                    "A guid names an item in Plex's catalogue; "
                    "a rating key names a row on this server"
                ),
                run(
                    ("search", "--track", "'<title>'"),
                    purpose="to get this server's rating key",
                ),
            ),
    ...
    if value.startswith("plex://"):
        raise UsageError(
            ...
            recovery=(
                note("Pass the number after the last slash, which is what `rating_key:` reports"),
                run((*words, "<rating_key>")),
            ),
    ...
    raise UsageError(
        ...
        recovery=(
            run((*words, "<rating_key>")),
            run(("search", "--artist", "'<name>'"), purpose="to find one"),
        ),
        code="BAD_RATING_KEY",
    )


def _command_words(command: Sequence[str]) -> tuple:
    """The caller's own words, refusing the one mistake that renders plausibly."""
    if isinstance(command, str):
        raise TypeError(
            "command is the words after the tool name, not a rendered invocation: "
            f"pass ('track',) rather than {command!r}"
        )
    words = tuple(command)
    if not words:
        raise ValueError("command names at least one word of the caller's own invocation")
    return words
...
def media_content_id(machine_identifier: str, rating_key) -> str:
    ...
    default server and is explicitly the legacy branch there; a tool that knows
    the machine identifier for free has no reason to emit the weaker one.
    """
    key = str(rating_key).strip()
    if not _RATING_KEY.match(key):
        # Unreachable through a CLI that validates first. Kept because the
...
def handoff(machine_identifier: str, item) -> dict:
    ...
    vendor-neutral because a tool built on this ships to anyone with a Plex
    library, and the output stops at the identifier because a suggestion about
    what plays it could only be assembled from something the caller told us --
    which is information they already had. The tool prints identifiers and
    stops; a ``play`` behind an explicit operator gate prints this block and
    then starts the item (see the module docstring).
    """
