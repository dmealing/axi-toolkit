...
| Upstream path | `tests/fixtures/encode/` inside the upstream clone — the vendored copy is `src/axi_core/toon_spec/encode/` |
| Spec version | 4.1.1 (SPEC.md v4.1, released 2026-08-05) |
| Commit | `62f16b369408180f1faf1cba7da1b46d1f336f12` |
| Licence | MIT — the copy in this repository is `src/axi_core/toon_spec/LICENSE`, from the same commit |
...
judges it. `axi_core.toon_spec.run(encode)` takes the encoder as an argument, so a tool
...
`axi_core.toon_spec.digest_mismatches()` is asserted by the suite: a fixture edited to
...
- **`decode/`** — `axi_core.toon` is an encoder only. Vendoring decode fixtures would
...
is `indent`. `axi_core.toon_spec.encoder_kwargs` maps one to the other in a single
...
cp <clone>/tests/fixtures/encode/*.json src/axi_core/toon_spec/encode/
cp <clone>/LICENSE src/axi_core/toon_spec/LICENSE
(cd src/axi_core/toon_spec/encode && sha256sum *.json) > src/axi_core/toon_spec/checksums.txt
