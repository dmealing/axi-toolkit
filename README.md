...
beside the encoder, and `axi_core.toon_spec.run(encode)` takes the encoder as an
...
| Module | What it is |
| --- | --- |
| `axi_core.toon` | A strict TOON encoder (spec v4.1). Encoding happens at the output boundary only. |
| `axi_core.toon_spec` | The specification's own encode fixtures, vendored and checksummed, plus the rig that runs them against *any* encoder. |
| `axi_core.errors` | `AxiError`, a code, the fault classes, and the closed recovery vocabulary. |
| `axi_core.render.cli` | Recovery intent → the line a shell user reads. Also `parse`, its inverse. |
| `axi_core.render.prose` | The same intent → a sentence naming the caller's own tool. |
| `axi_core.redact` | `register_secret`, `register_pattern`, and one filter with a documented ordering. |
| `axi_core.envconfig` | Environment-only credentials, control-character rejection, userinfo splitting. |
...
```python
from axi_core.errors import NotFound, run
...
checks from that declaration. `metaobjects/meta.axi-core.yaml` is the source of truth.
...
AXI_CORE_SOURCE_HA=<checkout> AXI_CORE_SOURCE_PLEX=<checkout> \
...
`src/axi_core/toon_spec/PROVENANCE.md`.
