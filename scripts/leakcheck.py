...
This package is the shared toolkit for AXI command-line tools, and it is built by
reading two of them and the installations they were developed against. That is
...
scanner that catches more in a public repository is strictly better than one
tuned to what this repository happens to hold today.
...
RULES = [
    Rule(
        ...
        "a JSON Web Token, which is what both tools' access tokens look like",
        condensed=True,
    ),
    Rule(
        "bearer",
...
PATH_ALLOWANCES = {
    # One upstream TOON case escapes backslashes in a synthetic Windows drive path
    # under the users directory. It names nobody and reaches nothing -- and the
    # shape is deliberately not repeated here, or this file would trip too. The
