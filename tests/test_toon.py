"""The encoder's behaviour, stated in this package's own words.

Not interchangeable with ``test_toon_conformance.py``, and both are kept. This suite
says what the encoder is for and pins the cases this project reasons about; that one
runs the specification's own fixtures. A rule nobody here thought to write a test for
reads as passing, which is how a released build once shipped two failing spec cases
while its README claimed strictness.

Carried over from the source tools' copy of the encoder, which is byte-identical in
both and which the vendored fixtures agree with.
"""
...
def test_a_small_reading_survives_the_document_boundary():
    """The reachable case: a measurement in either band, as a detail view prints it.

    Both are ordinary values a device reports -- a current in amps, a lifetime byte
    counter -- and both sit outside the range Python's float repr keeps decimal, which
    is why a repr-based encoder stops conforming exactly where real data lives.
    """
    doc = {
        "id": "sensor.example_current",
